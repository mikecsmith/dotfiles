...
def validate_config(raw_cfg):
    """
    Strictly checks for existence of required keys and
    validates that JQL templates won't fail during interpolation.
    """
    if "types" not in raw_cfg or not raw_cfg["types"]:
        raise ValueError("Missing 'types' array in config.yaml.")
...
def parse_config(raw_cfg):
    """Transforms validated raw data into the augmented app-ready version."""
    type_order_map = {
        str(t.get("id")): (
            int(t.get("order", 100)),
