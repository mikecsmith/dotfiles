...
---@class fathom.Toggle.wrap: fathom.Toggle
---@operator call:boolean

---@param toggle fathom.Toggle
function M.wrap(toggle)
  return setmetatable(toggle, {
    __call = function()
      ...
      if state then
        Fathom.info("Enabled " .. toggle.name, { title = toggle.name })
      else
        Fathom.warn("Disabled " .. toggle.name, { title = toggle.name })
      end
      return state
    end,
  }) --[[@as fathom.Toggle.wrap]]
end
...
function M.wk(lhs, toggle)
  local function safe_get()
    ...
    if not ok then
      Fathom.error({ "Failed to get toggle state for **" .. toggle.name .. "**:\n", enabled }, { once = true })
    end
    return enabled
  end
...
function M.format(buf)
  return M.wrap({
    ...
    get = function()
      ...
      return Fathom.format.enabled()
    end,
    set = function(state)
      Fathom.format.enable(state, buf)
    end,
  })
end
  ...
  set = function(state)
    if state then
      ...
      vim.api.nvim_create_autocmd("ExitPre", {
        once = true,
        group = vim.api.nvim_create_augroup("fathom_restore_max_exit_pre", { clear = true }),
