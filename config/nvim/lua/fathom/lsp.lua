...
function M.rename_file()
	...
	local old = assert(Fathom.root.realpath(vim.api.nvim_buf_get_name(buf)))
	local root = assert(Fathom.root.realpath(Fathom.root.get({ normalize = true })))
...
---@param opts? FathomFormatter| {filter?: (string|lsp.Client.filter)}
function M.formatter(opts)
	opts = opts or {}
	local filter = opts.filter or {}
	filter = type(filter) == "string" and { name = filter } or filter
	---@cast filter lsp.Client.filter
	---@type FathomFormatter
	local ret = {
		name = "LSP",
		primary = true,
		priority = 1,
		format = function(buf)
			M.format(Fathom.merge({}, filter, { bufnr = buf }))
		end,
		sources = function(buf)
			local clients = M.get_clients(Fathom.merge({}, filter, { bufnr = buf }))
			---@param client vim.lsp.Client
	...
	return Fathom.merge(ret, opts) --[[@as FathomFormatter]]
end
...
function M.format(opts)
	opts = vim.tbl_deep_extend(
		...
		{},
		opts or {},
		Fathom.opts("nvim-lspconfig").format or {},
		Fathom.opts("conform.nvim").format or {}
	)
			...
			callback = function(ev)
				...
				if not ({ M.words.get() })[2] then
					if ev.event:find("CursorMoved") then
						vim.lsp.buf.clear_references()
					elseif not Fathom.cmp.visible() then
